...
def custom_score(game, player):
    ...
    If the game state is terminal then it returns -inf or + inf depending
    on whether the player has one or lost.

    If the game state is not terminal (most cases) then it
    - gets the position of the player
    - gets the position of the opponent
    - caluclates the distance between player an dopponent
    
    Returns distance to incentivise player to keep his distance from the opponent.
...
def custom_score_2(game, player):
    ...
    Returns distance squared to STRONGLY incentivise player to keep his distance.
    ...
    #Return square of the distance between player 1 and player 2 so that
    #ditance is maximised
...
def custom_score_3(game, player):
    ...
    #Return the negative square of the distance between player 1 and player 2
    #so that distance is minimised rather than maximised
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def get_move(self, game, time_left):
        ...
        **********************************************************************
        ...
        try:
            ...
            #game tree to a specified depth and if we wtill have time we extedn the depth.
            #We continue doing this until we timeout and at that point we return what we
            #think is our best move.
            
            #Increment search depth until best move is found (timeout will occur after 24 moves)
    ...
    def min_value (self, game, depth, alpha, beta):
        ...
        #If we reached our depth limit then return a score for this player
        #ALfaBeta is implemented here using Iterative Deepening strategy so don't check for depth breach
        #as we expect a timeout (Minimax was implemented with Depth Limited search strategy)
        if depth <=0:
            print('Depth limit reached')
            return self.score(game, self)
...
#Caluclate score for this node by calling min function (next level down is min)
            score = self.min_value(next_state, depth -1, alpha, beta)
            if score > max_score:
                max_score = score
                best_move = move
