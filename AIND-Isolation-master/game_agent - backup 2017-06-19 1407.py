...
##    return float (10**math.sqrt((y1 - y2)**2 + (x1 - x2)**2))
    ...
    return float (-1*len(game.get_legal_moves(opponent)))
...
##    norm_blocks = (blocking_moves - min_blocks)/(max_blocks-min_blocks)
    ...
    return float(3*(len(player_moves))-(len(opponent_moves))-(10*player_average_distance)+opponent_average_distance+blocking_moves)
        ...
        **********************************************************************
        ...
        try:
            ...
            for x in range(0,1000000000000000):               
                ...
                if debug is True:print('Best move returned',best_move,'search depth=',self.search_depth+x)
            #return best_move
                
        except SearchTimeout:
            if debug is True:print('Time out reached',best_move,'search depth=',self.search_depth+x)
            pass
            #return best_move #previously obtained best move
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        for move in legal_moves:
            ...
            self.numberofnodesvisited +=1
            if debug is True:
                print('Testing next possible game state using min_value depth=',depth)
                print(next_state.to_string())

        #Calculate score for this node by calling min function (next level down is min)
        #If score for this node is highest so far at this level store it as the best move
            score = self.min_value(next_state, depth -1, alpha, beta)

            if debug is True:print('Score returned =',score,'Max_score at this level',max_score,'Alpha at this level',alpha)
            ...
            #If score is > Alpha (lower bound) then set Alpha to equal score
            #New Alpha will be used on subsequent iterations through game states at this level
            if   score > alpha:
                alpha = score
