...
##    norm_blocks = (blocking_moves - min_blocks)/(max_blocks-min_blocks)
    ...
    return float(3*(len(player_moves))-(len(opponent_moves))-(2*player_average_distance)+opponent_average_distance+blocking_moves)
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        #If score for this node is highest so far at this level store it as the best move
            ...
            #New Alpha will be used on subsequent iterations thrugh game states at this level
            if   score > alpha:
                alpha = score
                
        #If we don't have a best move but we have legal moves just play one of the legal moves
        if best_move==(-1,-1):
            legal_moves = game.get_legal_moves()
            if len(legal_moves)!=0:
                best_move=legal_moves[0]
