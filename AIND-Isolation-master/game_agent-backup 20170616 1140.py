...
def custom_score(game, player):
    ...
    in the belief that this will lead to having more squares avalable. Game
    States with the largest distance between the Player and Opponent score more
    highly. This assumes that by keeping our distance we will temd to have
    more movesavailable to us.
...
##    #maximised
##    return float (math.sqrt((y1 - y2)**2 + (x1 - x2)**2))

    #Score boards with fewest moves available to Opponent most highly
    #Get the opponent
    opponent = game.get_opponent(player)

#######    Reward boards where opponent has fewest moves##########################################

##    #Get list of available moves for opponent
##    opponent_moves=game.get_legal_moves(opponent)
##
##    #Count the number of the Opponent's avilable moves and subtract from 8 (the max number
##    #of moves possible) to return a high score when the Opponent has few moves and a Low score
##    #when Opponent has many moves
##    return float(8-(len(opponent_moves)))

#######    Reward boards where opponent has MOST moves - crazy!##########################################

##    #Get list of available moves for opponent
##    opponent_moves=game.get_legal_moves(opponent)
##
##    #Return the number of moves available to opponent 
##    return float(len(opponent_moves))

#######    STRONGLY Reward boards where opponent has MOST moves - multo crazy!#################
##    #Get list of available moves for opponent
##    opponent_moves=game.get_legal_moves(opponent)
##
##    #Return the number of moves available to opponent 
##    return float(2**len(opponent_moves))

#######    STRONGLY Reward boards where player has MOST moves#################
##    #Get list of available moves for opponent
##    player_moves=game.get_legal_moves(player)
##
##    #Return the number of moves available to opponent 
##    return float(2**len(player_moves))
...
##    #Get list of available moves for opponent
    ...
    #Return the number of moves available to opponent 
    return float(10**len(player_moves)-(len(opponent_moves)))
...
def custom_score_2(game, player):
    ...
    judged to be more peripheral most highly (I am assuming a peripheral position will tend
...
##    print('Normalised Average distance',norm_distance)
    return float(10**(opponent_average_distance) - player_average_distance)
...
class MinimaxPlayer(IsolationPlayer):
    ...
    def min_value (self, game, depth):
        ...
        #If we reached our depth limit then return a score for this player using a heuristic
        #evaluation function
        if depth <=0:
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def get_move(self, game, time_left):
        ...
        **********************************************************************
        ...
        if debug is True: print(game.to_string())
        ...
        try:
            ...
            #game tree to a specified depth and if we have time we generate it again
            #extending the depth by 1 level each time (we still use the depth restriction to limit
            #how deep we go each time). We continue doing this until we timeout
            #and at that point we return what we think is our best move (from previous
            #calculation)
            x=1
            while True:
                best_move = self.alphabeta(game, self.search_depth+x)
                if best_move == (-1,-1): #no moves available
                    return best_move
                saved_best_move = best_move
                if debug is True: print('Running to level',self.search_depth+x,'best move returned=',best_move)
                x=x+1
            if debug is True: print('Best move is',saved_best_move)
            return saved_best_move
        

        except SearchTimeout:   #If timeout exception thrown then use best move returned by last alfabeta
            if debug is True: print('Time out breached, returning previous best move =',saved_best_move)
            return saved_best_move 
           
        # Return the best move from the last completed search iteration
        return savedbest_move
    ...
    def min_value (self, game, depth, alpha, beta):
        ...
        if self.time_left() < self.TIMER_THRESHOLD:
            #print('Time out within Min_Value',self.time_left(),self.TIMER_THRESHOLD)
            raise SearchTimeout()
        ...
        if not legal_moves:
            utility = game.utility(None)
            if debug is True: print('utlity value is',utility)
            #print('No legal moves in Min at level',depth, 'of Alfabeta search',self.score(game,self))
            return utility
        
        if debug is True:print('Legal moves...',legal_moves)

        #If we reached our depth limit then return a score for this player based on our heuristic
        #evaluation function
        if depth <=0:
            if debug is True: print('Depth limit reached')
            print('depth limit reached returning heuristic score',self.score(game, self))
            return self.score(game, self)
        ...
        for move in legal_moves:
            ...
            #If score returned from next level down (Max) is the lowest recorded so far at this level (Min) then use it as min_score
            #for this level i.e. this is a minimising level so select lowest score returned 
            if score < min_score:
                min_score = score
    ...
    def max_value (self, game, depth, alpha, beta):
        ...
        #Abandon search if timeout breached
        print('Running alfabeta max_value at depth', depth,'for',game.active_player)
        ...
        if not legal_moves:
            utility = game.utility(None)
            if debug is True: print('utlity value is',utility)
            #print('No legal moves in Max at level',depth, 'of Alfabeta search',self.score(game,self))
            return utility

        if debug is True: print('Legal moves...',legal_moves)

        #If we reached our depth limit then return a score using our heuristic evaluation function
        if depth <=0:
            print('depth limit reached returning heuristic score',self.score(game, self))
            return self.score(game, self)
        ...
        for move in legal_moves:
            ...
            #If score returned from next level down (Min) is the highest recorded so far at this level then use it as max_score
            #for this level
            if score > max_score:
                max_score = score
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        This is an elaboration on the simple MiniMax above.
        ...
        **********************************************************************
        ...
        #Abandon search if timeout breached
        if self.time_left() < self.TIMER_THRESHOLD:
            if debug == True: print('Timeout reached')
            raise SearchTimeout()
        ...
        if not legal_moves:
            utility = game.utility(None)
            #if debug is True: print('utlity value is',utility)
            #print('No legal moves at level 0 of Alfabeta search',self.score(game,self))
            return utility

        if debug is True:print('Legal moves...',legal_moves)

        #If we reached our depth limit then return a score for this player using our heurisric evaluation function
        if depth <=0:
            if debug == True: print('depth limit reached')
            return self.score(game, self)
            
       
        
        #Generate a new game state for each move possible in legal_moves list and for each
        #   Call the min_value/ max_value functions to generate child nodes for each board and return a score
        #   back up the tree. Pass in alpha = -inf and beta = +inf on the inital call.
        #   Return the highest score returned from all child nodes below this level
        best_move=(-1,-1)
        #best_move=legal_moves[0] # Default best move to first legal move
        max_score = float("-inf")
        for move in legal_moves: #Loop round list of legal moves
            if debug is True: print('Next Action at level',depth,'=',move)
            next_state = game.forecast_move(move) #Apply move to current game
            self.numberofnodesvisited +=1
            if debug is True:
                print('Testing next game state using min_value for',game.active_player,'at level',depth)
                print(next_state.to_string(['1','2'],depth))

#Calculate score for this node by calling min function (next level down is min)
            score = self.min_value(next_state, depth -1, alpha, beta)
            if score > max_score:
                max_score = score
                best_move = move
