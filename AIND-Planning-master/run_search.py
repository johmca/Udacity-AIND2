...
def main(p_choices, s_choices):
    #p_choices is the option number entered on command line indicating problem to solve
    #s_choices is the option number entered on command line indicating search algo to use
    #Map these seections to the list of problems and search functions so that problems is a pointer to the function
    #corresponding to the selected problem e.g. function air_cargo_p1 at 0x000001891C2F6268> and searches is a
    #pointer to the selected search algo e.g.
    print('Running main..')
    ...
    for pname, p in problems:   #pname is text name of problem, p is pointer to function describing poblem

        for sname, s, h in searches: #sname is text name of search algo, s is pointer to function describing search algo
                                     # and I expect H to be something to do with heuristic?

            hstring = h if not h else " with {}".format(h)
            print("\nSolving {} using {}{}...".format(pname, sname, hstring))
            print('Creating problem')
            _p = p()  #Create new instance of problem (p points to functions defined in my_air_cargo_problems.py)
            _h = None if not h else getattr(_p, h) #create new instance of heuristic function
            print('Running search now')
            run_search(_p, s, _h)   #********************************************** RUN THE SEARCH *******
